	...
	public void pay_US_bill_ErrorMsg() throws Exception {
		...
		try {
			...
			if (platformName.equalsIgnoreCase("ios")) {
				...
				verify_message.isDisplayed();
				pay_bill_button.isDisplayed();
			} else {
		...
		try {
			...
			if (platformName.equalsIgnoreCase("ios")) {
				...
				if (!elementExists) {
					...
					boolean mobileElement = currency_switchbox.isDisplayed();
				...
				if (select_from_account.equalsIgnoreCase("US")) {
					...
					if (!switchBox)
						System.out.println("The Switch box is verified");
				} else {
