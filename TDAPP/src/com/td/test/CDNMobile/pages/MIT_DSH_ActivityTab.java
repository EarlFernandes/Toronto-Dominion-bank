...
public class MIT_DSH_ActivityTab extends _CommonPage {
	...
	public void verifyActivityDetailsUI() {
		...
		try {
			...
			// sXPathAnd =
			// "//android.widget.TextView[@resource-id='com.td:id/description']";

			/*
			 * for (WebElement meSymbol :
			 * CL.GetDriver().findElements(By.id("com.td:id/description"))) {
			 * 
			 * if
			 * ((mobileAction.FuncGetValByRegx(mobileAction.FuncGetElementText(
			 * meSymbol), "([0-9]+[' ']+[@])").length()>0)) {
			 * 
			 * 
			 * break;
			 * 
			 * } else {
			 * 
			 * iCnt++; mobileAction.FuncSwipeOnce("up"); } }
			 */
...

			{
				...
				// ')").length()>=1))&&(sprice.contains("$")))
				{
					...
					mobileAction.verifyTextContains(
							(MobileElement) CL.GetDriver().findElement(By.xpath(sXPathAndAmount)), "$");
					...
					CL.GetReporting().FuncReport("Pass",
							"Action and price is present, Amount is displayed and date/settle is displayed");
					...
					CL.GetReporting().FuncReport("Fail",
							"Action and price not present, Amount is not displayed and date/settle is not displayed");
				...
				// ')").length()>=1))&&(sprice.contains("$")))
				{
					...
					mobileAction.verifyTextContains(
							(MobileElement) CL.GetDriver().findElement(By.xpath(sXPathiOSAmount)), "$");
		 ...
		 * //////////////////////////////////////////////////////////
		 * 
		 * for (int i=0; i < 10; i++) { sXPathAnd =
		 * "//android.widget.TextView[@resource-id='com.td:id/description']";
		 * sXPathAndAmount =
		 * "//android.widget.TextView[@resource-id='com.td:id/amount']";
		 * sXPathAnddate =
		 * "//android.widget.TextView[@resource-id='com.td:id/date_settle']";
		 * if(mobileAction.verifyTextContains((MobileElement)CL.GetDriver().
		 * findElements(By.xpath(sXPathAnd)).get(i)," @")) {
		 * 
		 * String saction =
		 * CL.GetDriver().findElements(By.xpath(sXPathAnd)).get(i).getText().
		 * split("@")[0]; String sprice =
		 * CL.GetDriver().findElements(By.xpath(sXPathAnd)).get(i).getText().
		 * split("@")[1];
		 * 
		 * if(saction.contains("BUY")||saction.contains("SELL")&&(sprice.
		 * contains("$")))
		 * 
		 * 
		 * {
		 * 
		 * 
		 * mobileAction.verifyTextContains((MobileElement)CL.GetDriver().
		 * findElements(By.xpath(sXPathAndAmount)).get(i),"$");
		 * mobileAction.verifyTextContains((MobileElement)CL.GetDriver().
		 * findElements(By.xpath(sXPathAnddate)).get(i),"/");
		 * CL.GetReporting().FuncReport("Pass",
		 * "Action and price is present, Amount is displayed and date/settle is displayed"
		 * );
		 * 
		...
		/*
		 * //sXPathAnd =
		 * "//android.widget.TextView[@resource-id='com.td:id/description' and contains(@text,'"
		 * + des +"')]";
		 * 
		 * mobileAction.FuncSwipeWhileElementNotFoundByxpath(sXPathAnd, true,
		 * 60, "up");
		 * 
		 * } descReg =
		 * mobileAction.FuncGetValByRegx(mobileAction.FuncGetText(description),
		 * "([0-9]+[@])");
		 * 
		 * sXPathAnd =
		 * "//android.widget.TextView[@resource-id='com.td:id/description' and contains(@text,'"
