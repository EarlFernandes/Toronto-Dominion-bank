...
public class Currency extends _CommonPage {
	...
	private void init_currency_set() {
		Currency_Set_En.addAll(Arrays.asList(new String[] {"Aruba Florin (AWG)","Australian Dollar (AUD)",
				"Bahraini Dinar (BHD)","Barbados Dollar (BBD)","Bermuda Dollar (BMD)",
				"Brazil Real (BRL)","Bruneian Dollar (BND)","Bulgarian Lev (BGN)",
				"Caymanian Dollar (KYD)","Chilean Peso (CLP)","Chinese Renminbi (CNY)",
				"Colombian Peso (COP)","Costa Rican Colon (CRC)","Croatian Kuna (HRK)",
				"Czech Koruna (CZK)","Danish Krone (DKK)","Dominican Peso (DOP)",
				"Dutch Guilder (ANG)","East Caribbean Dollar (XCD)","Euro (EUR)",
				"Fiji Dollar (FJD)","Great Britain Pound (GBP)","Guatemala Quetzal (GTQ)",
				"Hong Kong Dollar (HKD)","Hungarian Forint (HUF)","Icelandic Krona (ISK)",
				"Indonesian Rupiah (IDR)","Jamaican Dollar (JMD)","Japanese Yen (JPY)",
				"Jordanian Dinar (JOD)","Kuwaiti Dinar (KWD)","Malaysian Ringgit (MYR)",
				"Mexican Peso (MXN)","New Israeli Sheqel (ILS)","New Zealand Dollar (NZD)",
				"Nicaragua Cordoba (NIO)","North Irish Pound (INP)","Norwegian Krone (NOK)",
				"Omani Rial (OMR)","Peruvian Nuevo Sol (PEN)","Philippine Peso (PHP)",
				"Polish Zloty (PLN)","Qatari Riyal (QAR)","Romanian Leu (RON)",
				"Russian Ruble (RUB)","Saudi Riyal (SAR)","Scottish Pound (SCO)",
				"Singapore Dollar (SGD)","South African Rand (ZAR)","South Korean Won (KRW)",
				"Swedish Krona (SEK)","Swiss Franc (CHF)","Tahitian Franc (XPF)",
				"Taiwan New Dollar (TWD)","Thai Baht (THB)","Turkish Lira (TRY)",
		...
		Currency_Set_FR.addAll(Arrays.asList(new String[] {"Baht thaïlandais (THB)","Colon costaricain (CRC)",
				...
				"Dollar singapourien (SGD)","Dong vietnamien (VND)","Euro (EUR)",
				"Florin arubais (AWG)","Florin néerlandais (ANG)","Forint hongrois (HUF)",
				"Franc CFP (XPF)","Franc suisse (CHF)","Kuna croate (HRK)",
				"Leu roumain (RON)","Lev bulgare (BGN)","Livre sterling (Irlande du Nord) (INP)",
				...
				"Peso colombien (COP)","Peso dominicain (DOP)","Peso mexicain (MXN)",
				"Peso philippin (PHP)","Peso uruguayen (UYU)","Quetzal guatémaltèque (GTQ)",
				"Rand sud-africain (ZAR)","Rial omanais (OMR)","Ringgit malaisien (MYR)",
				"Riyal qatarien (QAR)","Riyal saoudien (SAR)","Rouble russe (RUB)",
				"Roupie indonésienne (IDR)","Réal brésilien (BRL)","Shekel de l’État d’Israël (ILS)",
				"Won sud-coréen (KRW)","Yen japonais (JPY)","Yuan chinois (CNY)",
				"Zloty polonais (PLN)"}));
	}
	...
	public void VerifyDefaultCurrenyUSD() {
		...
		try {
			String usdXapth = "";
			if (CL.getTestDataInstance().getMobilePlatForm().equalsIgnoreCase("ios")) {
				usdXapth = "//XCUIElementTypeStaticText[contains(@label,'United States Dollar (USD)')]/..";
			} else {
				usdXapth = "//android.widget.TextView[contains(@text,'United States Dollar (USD)')]/../android.widget.ImageView[@resource-id='com.td:id/action_sheet_checkmark']";
			}
			...
			if (CL.getTestDataInstance().getMobilePlatForm().equalsIgnoreCase("ios")) {
				...
				if (usdLabel.matches("Selected\\s*United\\s*States\\s*Dollar")) {
