...
public class StockOptionLeg extends _CommonPage {
	...
	public void selectLeg2Action() {
		...
		try {
			...
			else if (secondLegAction.equalsIgnoreCase("Sell to Close")
					| secondLegAction.equalsIgnoreCase("Vente pour fermer"))
				mobileAction.FuncClick(actionSelltoClose, "Action - Buy to Open");
			else if (secondLegAction.equalsIgnoreCase("Buy to Close")
					| secondLegAction.equalsIgnoreCase("Achat pour fermer"))
				mobileAction.FuncClick(actionBuyToClose, "Action - Buy to Open");
			else if (secondLegAction.equalsIgnoreCase("Sell to Open Covered")
					| secondLegAction.equalsIgnoreCase("Vente pour ouvrir – couvert"))
				mobileAction.FuncClick(actionSelltoOpenCovered, "Action - Sell to Open Covered");
			else if (secondLegAction.equalsIgnoreCase("Sell to Open Uncovered")
					| secondLegAction.equalsIgnoreCase("Vente pour ouvrir – non couvert"))
	...
	// Selects Price Type Even from the list based on the value enter in the
	// data sheet
	...
	// Enters the Limit Price for a Multi-leg Stock/Option order. This has no
	...
	// Verifies the content of the Confirm Order screen for a Multi-leg
	// Stock/Option order
	...
	public void verifyConfirmOrder_ENG() {
		...
		try {
			if (mobileAction.isObjExists(lblFirstLeg) && (mobileAction.getText(lblFirstLegValue)
					.contains(firstLegDetail + " " + leg1qty_ENG + " " + symbol.toUpperCase()))) {
			...
			if (mobileAction.isObjExists(lblSecondLeg) && (mobileAction.getText(lblSecondLegValue)
					.contains(secondLegDetail + " " + leg2qty_ENG + " " + symbol.toUpperCase()))) {
			...
			if (mobileAction.isObjExists(lblPrice)
					&& (mobileAction.getText(lblPriceValue).matches(priceType + price_ENG))) {
			...
			if (mobileAction.isObjExists(lblEstimatedPrincipal) && (mobileAction.isObjExists(lblCommission))
					&& (mobileAction.isObjExists(lblEstimatedTotal))) {
				CL.GetReporting().FuncReport("Pass",
						"Labels - Estimated Principal, Commission and Estimated Total match!!");
			} else {
				CL.GetReporting().FuncReport("Fail",
						"Labels - Estimated Principal, Commission and Estimated Total DON'T match!!");
			}
			mobileAction.FuncSwipeOnce("up");

			if (this.shareholderFlag)
				if (mobileAction.isObjExists(lblShareholderType)
						&& (mobileAction.getText(lblShareholderTypeValue).matches(shareholderType))) {
					CL.GetReporting().FuncReport("Pass",
							"Shareholder Type exists and matches the details on the Confirm page");
				} else {
					CL.GetReporting().FuncReport("Fail",
							"Shareholder Type DOESN'T exist or not matches the details on the Confirm page");
				}
			else if (mobileAction.isObjExists(lblShareholderType)) {
				CL.GetReporting().FuncReport("Fail", "Shareholder Type exists on the Confirm page - FALSE POSITIVE");
			} else {
				CL.GetReporting().FuncReport("Pass",
						"Shareholder Type DOESN'T exist on the Confirm page - That's Good");
			}

			if (accountNumber.matches(".*E$") | accountNumber.matches(".*F$") | accountNumber.matches(".*G$")
					| accountNumber.matches(".*H$"))
				if (mobileAction.isObjExists(lblBuyingPower)) {
					CL.GetReporting().FuncReport("Pass", "Buying Power exists on the Confirm page");
				} else {
					CL.GetReporting().FuncReport("Fail", "Buying Power DOESN'T exist on the Confirm page");
				}
			else if (mobileAction.isObjExists(lblBuyingPower)) {
				CL.GetReporting().FuncReport("Fail", "Buying Power exists on the Confirm page - FALSE POSITIVE");
			} else {
				CL.GetReporting().FuncReport("Pass", "Buying Power DOESN'T exist on the Confirm page - That's Good");
			}
	...
	public void verifyConfirmOrder_FR() {
		...
		try {
			if (mobileAction.isObjExists(lblFirstLeg) && (mobileAction.getText(lblFirstLegValue)
					.contains(firstLegDetail + " " + leg1qty_FRE + " " + symbol.toUpperCase()))) {
				CL.GetReporting().FuncReport("Pass", "1st Leg order details match!!");
			} else {
				CL.GetReporting().FuncReport("Fail", "1st Leg order details DON'T match!!");
			}
			...
			if (mobileAction.isObjExists(lblSecondLeg) && (mobileAction.getText(lblSecondLegValue)
					.contains(secondLegDetail + " " + leg2qty_FRE + " " + symbol.toUpperCase()))) {
				CL.GetReporting().FuncReport("Pass", "2nd Leg order details match!!");
			} else {
				CL.GetReporting().FuncReport("Fail", "2nd Leg order details DON'T match!!");
			}
			...
			if (mobileAction.isObjExists(lblPrice)
					&& (mobileAction.getText(lblPriceValue).matches(priceType + price_FRE))) {
				CL.GetReporting().FuncReport("Pass", "Price details match!!");
			} else {
				CL.GetReporting().FuncReport("Fail", "Price details DON'T match!!");
			}
	...
	public void verifyReceiptScreen_ENG() {
		...
		try {
			...
			if (mobileAction.isObjExists(lblFirstLegConfirmation)
					&& (mobileAction.getText(lblFirstLegConfirmationNumber).matches("\\w{2}\\-\\d{6}"))) {
				CL.GetReporting().FuncReport("Pass",
						"1st Leg Confirmation details appear along with the Confirmation Number");
			} else {
				CL.GetReporting().FuncReport("Fail", "1st Leg Confirmation details DON'T match !!");
			}

			if (mobileAction.isObjExists(lblSecondLegConfirmation)
					&& (mobileAction.getText(lblSecondLegConfirmationNumber).matches("\\w{2}\\-\\d{6}"))) {
				CL.GetReporting().FuncReport("Pass",
						"2nd Leg Confirmation details appear along with the Confirmation Number");
			} else {
				CL.GetReporting().FuncReport("Fail", "2nd Leg Confirmation details DON'T match !!");
			}
			if (mobileAction.isObjExists(lblReceipt_FirstLeg) && (mobileAction.getText(lblReceipt_FirstLegValue)
					.contains(firstLegDetail + " " + leg1qty_ENG + " " + symbol.toUpperCase()))) {
				CL.GetReporting().FuncReport("Pass", "1st Leg order details match!!");
			} else {
				CL.GetReporting().FuncReport("Fail", "1st Leg order details DON'T match!!");
			...
			if (mobileAction.isObjExists(lblReceipt_SecondLeg) && (mobileAction.getText(lblReceipt_SecondLegValue)
					.contains(secondLegDetail + " " + leg2qty_ENG + " " + symbol.toUpperCase()))) {
				CL.GetReporting().FuncReport("Pass", "2nd Leg order details match!!");
			} else {
				CL.GetReporting().FuncReport("Fail", "2nd Leg order details DON'T match!!");
			...
			if (mobileAction.isObjExists(lblReceipt_Price)
					&& (mobileAction.getText(lblReceipt_PriceValue).matches(priceType + price_ENG))) {
				CL.GetReporting().FuncReport("Pass", "Price details match!!");
			} else {
				CL.GetReporting().FuncReport("Fail", "Price details DON'T match!!");
			}

			if (mobileAction.isObjExists(lblReceipt_GoodTil)
					&& (mobileAction.getText(lblReceipt_GoodTilValue).matches(goodtil))) {
				CL.GetReporting().FuncReport("Pass", "Good Til details match!!");
			} else {
				CL.GetReporting().FuncReport("Fail", "Good Til details DON'T match!!");
			}
			...
			if (this.shareholderFlag)
				if (mobileAction.isObjExists(lblReceipt_ShareholderType)
						&& (mobileAction.getText(lblReceipt_ShareholderTypeValue).matches(shareholderType))) {
					CL.GetReporting().FuncReport("Pass",
							"Shareholder Type exists and matches the details on the Receipt page");
				} else {
					CL.GetReporting().FuncReport("Fail",
							"Shareholder Type DOESN'T exist or not matches the details on the Receipt page");
				}
			else if (mobileAction.isObjExists(lblReceipt_ShareholderType)) {
				CL.GetReporting().FuncReport("Fail", "Shareholder Type exists on the Receipt page - FALSE POSITIVE");
			} else {
				CL.GetReporting().FuncReport("Pass",
						"Shareholder Type DOESN'T exist on the Receipt page - That's Good");
			}

			if (mobileAction.isObjExists(btnHome) && mobileAction.isObjExists(btnOrders)
					&& mobileAction.isObjExists(btnTrade)) {
	...
	public void verifyReceiptScreen_FRE() {
		...
		try {
			...
			if (mobileAction.isObjExists(lblReceipt_Price)
					&& (mobileAction.getText(lblReceipt_PriceValue).matches(priceType + price_FRE))) {
				CL.GetReporting().FuncReport("Pass", "Price details match!!");
			} else {
				CL.GetReporting().FuncReport("Fail", "Price details DON'T match!!");
			}
